...
while True:
    ...
    for result in results:
        ...
        logging.info(date)
        cur.execute('select status from sensor where sensorID = ' + str(result[0]))
        status = cur.fetchall()
        status = status[0][0]
        logging.info(status)
        if now - timedelta(minutes=2) > date and status == "online":
            ...
            cur.execute('update sensor set status = \'offline\' where sensorID = ' + str(result[0]))

            #sms versenden
            with open("/var/spool/sms/outgoing/sensor_offline.txt", mode='w') as f:
                cur.execute('select sensorPosition from sensor where sensorID = ' + str(result[0]))
